...
public class SevenZipExtractorProvider : IArchiveExtractorProvider
{
    ...
    public async Task<bool> ExtractAsync(string archivePath, string destinationPath, CancellationToken cancellationToken = default)
    {
        ...
        if (binary == null)
        {
            _logger.Error("7-Zip binary not found on host. Cannot perform native extraction of '{0}'.", archivePath);
            return false;
        }
        ...
        try
        {
            _logger.Info("7-Zip extracting '{0}' to '{1}' using '{2}'...", archivePath, targetDir, binary);
            ...
            {
                _logger.Info("7-Zip successfully extracted archive '{0}'.", archivePath);
                return true;
            }

            var stderr = await process.StandardError.ReadToEndAsync(cancellationToken);
            _logger.Warn("7-Zip extraction finished with exit code {0}: {1}", process.ExitCode, stderr);
            return false;
        }
        catch (OperationCanceledException)
        {
            _logger.Warn("7-Zip extraction of '{0}' was canceled.", archivePath);
            throw;
        }
        catch (Exception ex)
        {
            _logger.Error(ex, "7-Zip failed to extract archive: {0}", archivePath);
            return false;
        }
    }
