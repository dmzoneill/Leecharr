    ...
    IDisposable
{
    ...
    public void RejectRequest(int torrentId, int pieceIndex, int blockOffset, string peerId = null)
    {
        if (this.tasks.TryGetValue(torrentId, out var task))
        {
            task.RejectRequest(pieceIndex, blockOffset, peerId);
        }
    }

    public void RejectRequest(int torrentId, int pieceIndex, int blockOffset, int length, string peerId = null)
    {
        if (this.tasks.TryGetValue(torrentId, out var task))
        {
            task.RejectRequest(pieceIndex, blockOffset, length, peerId);
        }
    }
    ...
    private async Task StartEngineAsyncCore()
    {
        ...
        var engineSettingsBuilder = new EngineSettingsBuilder
        {
            ...
            StaleRequestTimeout = TimeSpan.FromSeconds(this.configService.StaleRequestTimeoutSeconds > 0 ? this.configService.StaleRequestTimeoutSeconds : 20),
            MaximumHalfOpenConnections = this.configService.MaximumHalfOpenConnections > 0 ? this.configService.MaximumHalfOpenConnections : 50,
    ...
    public async Task<IDownloadTask> AddTorrentAsync(CoreTorrent torrent, byte[] torrentFileBytes = null, string magnetUri = null)
    {
        ...
        finally
        {
            if (this.inFlightAdds.TryRemove(torrent.Id, out var removedCts))
            {
                ...
                {
                    this.logger.Trace(ex, "CancellationTokenSource already disposed during cleanup");
                }
            }
    ...
    public async Task RemoveTorrentAsync(int torrentId, bool deleteFiles)
    {
        if (this.inFlightAdds.TryGetValue(torrentId, out var inFlightCts))
        {
            ...
            {
                this.logger.Trace(ex, "In-flight CTS already disposed during removal");
            }
    ...
    public async Task OnTorrentCompletedAsync(int torrentId, string infoHash, TorrentManager manager)
    {
        ...
        var currentSavePath = existingTask?.SavePath ?? currentIncompletePath;
        var currentWorkingPath = existingTask?.WorkingPath ?? currentIncompletePath;

        var sourcePath = currentIncompletePath;
        ...
        var needsRelocation = !string.IsNullOrWhiteSpace(seedingSavePath) &&
        ...
        if (needsRelocation)
        {
            var availableSpace = this.diskProvider.GetAvailableSpace(seedingSavePath);
            var totalSize = manager.Torrent?.Size ?? existingTask?.TotalBytes ?? 0L;

            if (availableSpace.HasValue && totalSize > 0 && availableSpace.Value < totalSize)
            {
                this.logger.Error(
                    "Cannot relocate completed torrent #{0} ('{1}'): Insufficient disk space on target destination '{2}'. Required: {3} bytes, Available: {4} bytes.",
                    torrentId,
                    torrentName,
                    seedingSavePath,
                    totalSize,
            ...
            else
            {
                ...
                if (moveSucceeded)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        this.logger.Error(
                            ex,
                            "File relocation failed for torrent #{0} ('{1}'): MonoTorrent MoveFilesAsync to '{2}' threw an exception.",
                            torrentId,
                            torrentName,
                            seedingSavePath);
                    }
        ...
        {
            finalDestination ??= currentIncompletePath;

            if (existingTask != null)
            {
                existingTask.WorkingPath = currentIncompletePath;
    ...
    public static void TrimMonoTorrentMassiveBuffers()
    {
        ...
        {
            StaticLogger.Trace(ex, "Failed to clear internal buffer pool queue");
        }
    }
        ...
        out long writeRate)
    {
        ...
        {
            MonoTorrentDownloadEngine.StaticLogger.Trace(ex, "Failed to read internal MonoTorrent disk IO telemetry properties");
        }
    }
    ...
    private void PopulateDynamicEngineSettings(EngineSettingsBuilder builder, Dictionary<string, IPEndPoint> listenEndPoints = null)
    {
        ...
        builder.StaleRequestTimeout = TimeSpan.FromSeconds(this.configService.StaleRequestTimeoutSeconds > 0 ? this.configService.StaleRequestTimeoutSeconds : 20);
    ...
    internal static void ConfigureGlobalMonoTorrentDefaults(string prefix, string userAgent = null, bool anonymousMode = false)
    {
        ...
        try
        {
            ...
            foreach (var asmName in knownMonoTorrentAssemblyNames)
            {
                ...
                {
                    StaticLogger.Trace(ex, "MonoTorrent assembly was not loaded");
                }
            }
            ...
            foreach (var asm in monoTorrentAssemblies)
            {
                ...
                {
                    StaticLogger.Trace(ex, "Failed to patch MonoTorrent.GitInfoHelper");
                }
                ...
                {
                    StaticLogger.Trace(ex, "Failed to patch MonoTorrent.Dht.Messages.DhtMessage");
                }
                ...
                {
                    StaticLogger.Trace(ex, "Failed to patch ExtendedHandshakeMessage");
                }
            }
        }
        catch (Exception ex)
        {
            StaticLogger.Trace(ex, "Failed to configure MonoTorrent static version info");
        }
    }
    ...
    private static void SetStaticField(FieldInfo field, object value)
    {
        ...
        {
            StaticLogger.Trace(ex, "Failed to invoke SetStaticField via dynamic method");
        }
    }
    ...
    public long GetEffectiveMemoryBytes(string cgroupV2Path = "/sys/fs/cgroup/memory.max", string cgroupV1Path = "/sys/fs/cgroup/memory/memory.limit_in_bytes")
    {
        ...
        {
            this.logger.Trace(ex, "GC memory info unavailable, fallback to default 8GB");
        }
...
public class MonoTorrentDownloadTask : IDownloadTask
{
    ...
    {
        this.Picker?.RejectRequest(pieceIndex, blockOffset, peerId);
    }

    public void RejectRequest(int pieceIndex, int blockOffset, int length, string peerId = null)
    {
        this.Picker?.RejectRequest(pieceIndex, blockOffset, length, peerId);
    }
    ...
    private void OnPeerConnected(object sender, PeerConnectedEventArgs e)
    {
        try
        {
            ...
            {
                this.logger.Trace(ex, "Peer client string could not be formatted on connect");
            }
    ...
    private void OnPeerDisconnected(object sender, PeerDisconnectedEventArgs e)
    {
        try
        {
            ...
            {
                this.logger.Trace(ex, "Peer client string could not be formatted on disconnect");
            }
    ...
    internal static void TeardownPeerConnection(object peer)
    {
        ...
        {
            MonoTorrentDownloadEngine.StaticLogger.Trace(ex, "Peer connection disposal error during teardown");
        }
    }
    ...
    private IList<PeerId> GetCachedPeers()
    {
        ...
        if (shouldUpdate)
        {
            Task.Run(async () =>
            {
                ...
                {
                    this.logger.Trace(ex, "Failed to query peers asynchronously from manager for {0}", this.TorrentId);
                }
                finally
                {
                    lock (this.peerLock)
    ...
    public int[] PieceAvailability
    {
        get
        {
            ...
            {
                this.logger.Trace(ex, "Failed to compute piece availability across active peers for {0}", this.TorrentId);
            }
    ...
    private static bool IsPeerIncoming(PeerId peer)
    {
        ...
        {
            MonoTorrentDownloadEngine.StaticLogger.Trace(ex, "Reflection reading incoming peer direction threw");
        }
...
public class FilteringPeerConnectionListener : MonoTorrent.Connections.Peer.IPeerConnectionListener
{
    ...
    private void OnInnerConnectionReceived(object sender, MonoTorrent.Connections.Peer.PeerConnectionEventArgs e)
    {
        if (this.isHalted?.Invoke() == true)
        {
            ...
            {
                MonoTorrentDownloadEngine.StaticLogger.Trace(ex, "Gated connection disposal failed when halted");
            }
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(ip) && this.blocklistService != null && this.blocklistService.IsIpBlocked(ip))
            {
                ...
                {
                    MonoTorrentDownloadEngine.StaticLogger.Trace(ex, "Blocked peer connection disposal failed");
                }
        ...
        {
            MonoTorrentDownloadEngine.StaticLogger.Trace(ex, "URI or EndPoint formatting failed for peer connection");
        }
        ...
        if (!string.IsNullOrEmpty(ip) && this.maxConnectionsPerIp > 0)
        {
            ...
            if (ipCount > this.maxConnectionsPerIp)
            {
                ...
                {
                    MonoTorrentDownloadEngine.StaticLogger.Trace(ex, "Rate-limited connection disposal failed");
                }
        ...
        if (Interlocked.Increment(ref this.halfOpenCount) > this.maxHalfOpenConnections)
        {
            ...
            {
                MonoTorrentDownloadEngine.StaticLogger.Trace(ex, "Max half-open connections exceeded, disposal failed");
            }
...
public sealed class HandshakeMonitoredPeerConnection : MonoTorrent.Connections.Peer.IPeerConnection, IDisposable
{
        ...
        IConfigService configService = null)
    {
        ...
        this.timeoutCts.Token.Register(() =>
        {
            if (Interlocked.CompareExchange(ref this.completedOrDisposed, 1, 0) == 0)
            {
                ...
                {
                    MonoTorrentDownloadEngine.StaticLogger.Trace(ex, "Handshake timeout inner connection disposal failed");
                }
    ...
    public async ReusableTasks.ReusableTask<int> ReceiveAsync(Memory<byte> buffer)
    {
        ...
        if (this.configService != null && !this.configService.ExtensionFastExtension && buffer.Length >= 28 && buffer.Span[0] == 19)
        {
            buffer.Span[27] &= unchecked((byte)~0x04);
        }
        ...
        if (Volatile.Read(ref this.completedOrDisposed) == 0)
        {
            ...
            if (total >= 68 && Interlocked.CompareExchange(ref this.completedOrDisposed, 1, 0) == 0)
            {
                ...
                {
                    MonoTorrentDownloadEngine.StaticLogger.Trace(ex, "Timeout CTS already disposed on handshake completion");
                }
    ...
    public ReusableTasks.ReusableTask<int> SendAsync(Memory<byte> buffer)
    {
        if (this.configService != null && !this.configService.ExtensionFastExtension && buffer.Length >= 28 && buffer.Span[0] == 19)
        {
            buffer.Span[27] &= unchecked((byte)~0x04);
        ...
        return this.inner.SendAsync(buffer);
    }
    ...
    public void Dispose()
    {
        if (Interlocked.CompareExchange(ref this.completedOrDisposed, 1, 0) == 0)
        {
            ...
            {
                MonoTorrentDownloadEngine.StaticLogger.Trace(ex, "Timeout CTS already disposed on connection disposal");
            }
        ...
        if (Interlocked.CompareExchange(ref this.isDisposed, 1, 0) == 0)
        {
            ...
            {
                MonoTorrentDownloadEngine.StaticLogger.Trace(ex, "Inner connection disposal failed during handshake connection dispose");
            }
            ...
            {
                MonoTorrentDownloadEngine.StaticLogger.Trace(ex, "Callback invoked on disposal threw");
            }
        }
    }
}
...
public class BoundSocketConnector : MonoTorrent.Connections.ISocketConnector
{
    ...
    public Socket CreateDatagramSocket(AddressFamily addressFamily = AddressFamily.InterNetwork, int localPort = 0)
    {
        ...
        if (this.configService != null && !this.configService.UtpEnabled)
        {
    ...
    public async ReusableTasks.ReusableTask<System.Net.Sockets.Socket> ConnectAsync(Uri uri, CancellationToken token)
    {
        ...
        if (isDatagram && this.configService != null && !this.configService.UtpEnabled)
        {
            if (this.configService.TcpFallback)
            {
                isDatagram = false;
            }
            else
        ...
        foreach (var address in addresses)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (isDatagram && (this.configService?.TcpFallback ?? true))
                {
                    try
                    {
                        var tcpSocket = new System.Net.Sockets.Socket(address.AddressFamily, SocketType.Stream, ProtocolType.Tcp);
                        this.BindSocket(tcpSocket, 0);
                        await tcpSocket.ConnectAsync(new IPEndPoint(address, uri.Port), token).ConfigureAwait(false);
                        return tcpSocket;
                    }
