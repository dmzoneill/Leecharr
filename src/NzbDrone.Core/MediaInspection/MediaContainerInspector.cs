...
public class MediaContainerInspector : IMediaContainerInspector
{
    public MediaContainerInfo InspectFile(string filePath)
    {
        ...
        return Inspect(stream, Path.GetFileName(filePath));
    }
    ...
    private static MediaContainerInfo InspectFlac(byte[] header)
    {
        ...
        if (header.Length >= 22)
        {
            // STREAMINFO block is immediately after 'fLaC' (offset 4) + 4-byte header
            // Bytes 18-20 contain sample rate (20 bits), channels (3 bits), bits per sample (5 bits)
            var b18 = header[18];
            var b19 = header[19];
            var b20 = header[20];
    ...
    private static MediaContainerInfo InspectByFileName(string fileName)
    {
    ...
    private static void ApplyFilenameHints(MediaContainerInfo info, string fileName)
    {
        if (string.IsNullOrWhiteSpace(fileName))
        {
            return;
