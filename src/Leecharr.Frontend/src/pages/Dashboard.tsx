...
import { useArrConnections, useIndexers, useSchedulerConfig } from '../api/hooks';
...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  const getArrStatus = (serviceName: string) => {
    ...
    const conn = list.find((c) => {
      ...
      const typeMatch = c.arrType?.toLowerCase().includes(serviceName.toLowerCase()) ||
  ...
  const ecosystemItems = [
    ...
    { name: 'Deluge RPC', icon: '⚡', status: { label: 'Port 7889 (Active)', color: 'var(--success, #22c55e)' } },
    { name: 'qBittorrent API', icon: '🔌', status: { label: 'Port 7889 (Active)', color: 'var(--success, #22c55e)' } },
  ];
    ...
    <div className="dashboard-page" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
          ...
          <div>
            <div style={{ fontWeight: 700, color: 'var(--accent, #ffd166)', fontSize: '1rem' }}>
              Level 3: Swarm Leecher & Peer Accelerant{' '}
              <span style={{ color: 'var(--text-muted, #7e8092)', fontSize: '0.8rem', fontWeight: 500 }}>
                3/10 BADGES
              </span>
            </div>
            <div style={{ color: 'var(--text-secondary, #c7c5d3)', fontSize: '0.85rem', marginTop: '2px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1.25rem' }}>
        <div className="card" style={{ padding: '1.25rem', textAlign: 'center', borderRadius: '8px' }}>
          <div style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--text-primary, #f8f4ed)' }}>
            {torrents.length}
          </div>
          <div
            style={{
        ...
        <div className="card" style={{ padding: '1.25rem', textAlign: 'center', borderRadius: '8px' }}>
          <div style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--accent, #ffd166)' }}>
        ...
        <div className="card" style={{ padding: '1.25rem', textAlign: 'center', borderRadius: '8px' }}>
          <div style={{ fontSize: '2rem', fontWeight: 700, color: 'var(--success, #22c55e)' }}>
      ...
      <div className="card" style={{ padding: '1rem 1.25rem', borderRadius: '8px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
          <span style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--text-primary, #f8f4ed)' }}>
            Connected Ecosystem
          </span>
          <span
            style={{ fontSize: '0.8rem', color: 'var(--accent, #ffd166)', cursor: 'pointer' }}
        ...
        <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          {ecosystemItems.map((item, idx) => (
            ...
            >
              ...
              <div>
                <div style={{ fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-primary, #f8f4ed)' }}>
                  {item.name}
                </div>
                <div
                  style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1.25rem' }}>
        ...
        <div className="card" style={{ padding: '1.25rem', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
          ...
          <div>
            <div style={{ fontSize: '0.85rem', color: 'var(--accent, #ffd166)', fontWeight: 600, marginBottom: '4px' }}>
              &bull; Downloading: {torrents.filter((t) => t.status === 'downloading').length}
            </div>
            <div style={{ fontSize: '0.85rem', color: 'var(--success, #22c55e)', fontWeight: 600, marginBottom: '4px' }}>
              &bull; Seeding: {torrents.filter((t) => t.status === 'seeding').length}
            </div>
            <div style={{ fontSize: '0.85rem', color: 'var(--text-muted, #7e8092)', fontWeight: 600 }}>
        ...
        <div className="card" style={{ padding: '1.25rem', borderRadius: '8px' }}>
          <div style={{ fontSize: '0.85rem', fontWeight: 600, color: 'var(--text-primary, #f8f4ed)', marginBottom: '0.75rem' }}>
            Speed Schedule
          </div>
          <div
            style={{
          ...
          >
            ...
            <span className="badge" style={{ backgroundColor: 'var(--bg-primary, #10111a)', color: 'var(--text-secondary, #c7c5d3)' }}>
          ...
          >
            ...
            <span style={{ color: 'var(--text-primary, #f8f4ed)', fontWeight: 600 }}>
              {schedulerConfig?.uploadLimitKBs ? `${schedulerConfig.uploadLimitKBs} KB/s` : 'Unlimited'}
            </span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', padding: '4px 0' }}>
            ...
            <span style={{ color: 'var(--text-primary, #f8f4ed)', fontWeight: 600 }}>
              {schedulerConfig?.downloadLimitKBs ? `${schedulerConfig.downloadLimitKBs} KB/s` : 'Unlimited'}
            </span>
          </div>
        </div>
        ...
        <div className="card" style={{ padding: '1.25rem', borderRadius: '8px' }}>
          ...
          {activeIndexers.length > 0 ? (
            activeIndexers.slice(0, 2).map((idx) => (
              ...
              >
                <span style={{ color: 'var(--text-secondary, #c7c5d3)' }}>{idx.name} ({idx.indexerType}) ↗</span>
            ...
            >
              ...
              <span
                style={{ color: 'var(--accent, #ffd166)', cursor: 'pointer', fontWeight: 600 }}
          ...
          {activeTrackerDomains.length > 0 ? (
            activeTrackerDomains.map((domain, i) => (
              <div
                ...
                style={{
                  ...
                  fontSize: '0.8rem',
                  padding: '4px 0',
                  borderBottom: i < activeTrackerDomains.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none',
                }}
              >
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '0.8rem', padding: '4px 0' }}>
              <span style={{ color: 'var(--text-secondary, #c7c5d3)' }}>BitTorrent DHT Swarm ↗</span>
      ...
      <div className="card" style={{ padding: '1.25rem', borderRadius: '8px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ fontSize: '0.9rem', fontWeight: 600, color: 'var(--text-primary, #f8f4ed)' }}>Transfer Speed</span>
            <span
              className="badge"
              style={{
