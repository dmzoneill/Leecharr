...
export const Indexers: React.FC<IndexersProps> = ({
  ...
  const handleToggleEnable = (indexer: IndexerDefinition) => {
    updateIndexerMutation.mutate(
      ...
      {
        onSuccess: () => {
          showToast(`Indexer "${indexer.name}" ${!indexer.enable ? 'enabled' : 'disabled'}`, 'success');
        },
      }
    );
  };
  ...
  const filtered = freeleechOnly
    ? rawResults.filter((r) => r.isFreeleech || (r.downloadVolumeFactor !== undefined && r.downloadVolumeFactor === 0))
    : rawResults;
      ...
      >
        <div>
          <h1 className="page-heading" style={{ margin: 0, fontSize: '1.4rem' }}>
            {currentIndexer ? `Indexer: ${currentIndexer.name}` : 'Multi-Indexer Search & Discovery'}
          </h1>
      ...
      >
        <form onSubmit={handleSearch} style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
          ...
          <button
            ...
            style={{ padding: '0.6rem 1.5rem', fontWeight: 600, borderRadius: '6px' }}
          >
            {searchResults.isFetching ? 'Searching swarms...' : 'Search Releases'}
          </button>
        </form>

        <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center', marginTop: '0.75rem', fontSize: '0.85rem' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', cursor: 'pointer', color: 'var(--text-secondary, #c7c5d3)' }}>
            <input
              type="checkbox"
        ...
        <div style={{ flex: '1 1 auto', minHeight: 0, overflowY: 'auto' }}>
          {searchResults.isFetching && (
            <div style={{ padding: '4rem', textAlign: 'center' }}>
              <div className="loading" style={{ fontSize: '1rem', color: 'var(--accent, #ffd166)' }}>
                Querying {currentIndexer ? currentIndexer.name : `${enabledIndexers.length} active indexers`}...
              </div>
            </div>
          )}
          ...
          {searchResults.isError && (
            <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--danger, #ef4444)' }}>
              Search query failed: {(searchResults.error as Error)?.message || 'Check indexer connection'}
            </div>
          )}

          {!searchResults.isFetching && !searchResults.isError && !activeSearchTerm && (
            <div style={{ padding: '4rem 2rem', textAlign: 'center', color: 'var(--text-muted, #7e8092)' }}>
              <div style={{ fontSize: '2.5rem', marginBottom: '0.75rem' }}>🔍</div>
              <div style={{ fontWeight: 600, fontSize: '1.05rem', color: 'var(--text-primary, #f8f4ed)', marginBottom: '0.25rem' }}>
                {currentIndexer ? `Ready to search ${currentIndexer.name}` : 'Ready to search all indexers'}
              </div>
          ...
          {!searchResults.isFetching && !searchResults.isError && activeSearchTerm && filtered.length === 0 && (
            <div style={{ padding: '4rem 2rem', textAlign: 'center', color: 'var(--text-muted, #7e8092)' }}>
              <div style={{ fontSize: '2.5rem', marginBottom: '0.75rem' }}>📭</div>
              <div style={{ fontWeight: 600, fontSize: '1.05rem', color: 'var(--text-primary, #f8f4ed)', marginBottom: '0.25rem' }}>
                No releases found for "{activeSearchTerm}"
              </div>
              <p style={{ maxWidth: '440px', margin: '0 auto', fontSize: '0.85rem' }}>
            ...
            <table className="table" style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {filtered.map((rel) => {
                  ...
                  const isFree = rel.isFreeleech || (rel.downloadVolumeFactor !== undefined && rel.downloadVolumeFactor === 0);
                  ...
                  return (
                    ...
                    >
                      <td style={{ padding: '0.75rem 1rem' }}>
                        <div style={{ fontWeight: 500, color: 'var(--text-primary, #f8f4ed)', wordBreak: 'break-word' }}>
                          {rel.title}
                          {isFree && (
                            <span
                              className="badge"
                      ...
                      <td style={{ padding: '0.75rem 1rem', whiteSpace: 'nowrap' }}>
                        ...
                        <span style={{ color: 'var(--text-muted, #7e8092)', marginLeft: '0.25rem' }}>
                          ▼ {rel.leechers ?? 0}
                        </span>
                      </td>
