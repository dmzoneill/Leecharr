...
export const PieceMapModal: React.FC<PieceMapModalProps> = ({ torrent, onClose }) => {
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content piece-map-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <div className="piece-map-stats">
            <span><strong>Total Pieces:</strong> {torrent.pieceCount || 'Unknown'}</span>
            <span><strong>Piece Length:</strong> {Math.round((torrent.pieceLength || 0) / 1024)} KB</span>
            <span><strong>Downloaded:</strong> {Math.round((torrent.progress || 0) * 100)}%</span>
          </div>
          <div className="canvas-wrapper">
            <canvas ref={canvasRef} width={600} height={260} className="piece-canvas" />
          </div>
          <div className="piece-legend">
            <span className="legend-item"><span className="legend-box piece-done"></span> Completed</span>
            <span className="legend-item"><span className="legend-box piece-active"></span> In-Flight</span>
            <span className="legend-item"><span className="legend-box piece-missing"></span> Missing</span>
          </div>
        </div>
      </div>
    </div>
