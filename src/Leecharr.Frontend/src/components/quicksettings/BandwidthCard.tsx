...
export const BandwidthCard: React.FC = () => {
  ...
  const handleUpdate = (updates: Partial<import("../../api/types").SeedingConfig>) => {
    if (!config) return;
    saveMutation.mutate({
      ...config,
      ...updates,
    ...
    <div className="quick-card">
      <div className="quick-card-header">
        ...
        <button
          ...
          onClick={() => handleUpdate({ alternativeSpeedEnabled: !isAltActive })}
      ...
      <div className="quick-card-body">
        ...
        <div className="quick-control-row">
          <div className="quick-control-label">
            ...
            <span className="quick-control-current">
              {currentDl === 0 ? "∞ Unlimited" : `${currentDl >= 1000 ? (currentDl / 1000).toFixed(1) + " MB/s" : currentDl + " KB/s"}`}
            </span>
          </div>
            ...
            {DL_PRESETS.map((p) => {
              ...
              return (
                <button
                  ...
                  onClick={() => handleUpdate({ maxDownloadSpeedKbps: p.value })}
                >
                  {p.label}
                </button>
              );
        ...
        <div className="quick-control-row">
          <div className="quick-control-label">
            ...
            <span>Max UL:</span>
            <span className="quick-control-current">
              {currentUl === 0 ? "∞ Unlimited" : `${currentUl >= 1000 ? (currentUl / 1000).toFixed(1) + " MB/s" : currentUl + " KB/s"}`}
            </span>
          </div>
          <div className="quick-presets-group">
            {UL_PRESETS.map((p) => {
        ...
        {isAltActive && (
          <div className="quick-alt-speed-banner">
            <span style={{ fontSize: "0.75rem", color: "var(--accent, #ffd166)", fontWeight: 600 }}>
              🐢 Alternative limits active:
            </span>
            <span style={{ fontSize: "0.75rem", color: "var(--text-secondary)" }}>
              DL: {config.altDownloadSpeedKbps ?? 2000} KB/s | UL: {config.altUploadSpeedKbps ?? 500} KB/s
            </span>
          </div>
        )}
      </div>
