 ...
 */
export function calculateHnrStatus(torrent: Torrent, requiredHours: number = 72): HnrStatus {
  ...
  const progressPercent = Math.min(99.9, (seededSeconds / requiredSeconds) * 100);
...
export function getTorrentBadges(
  ...
  if ((torrent.seeders ?? 0) <= 2 && (torrent.status || "").toLowerCase() === "seeding") {
    badges.push({
      label: "Guardian",
      icon: "🛡️",
...
export function calculateAchievements(
  ...
  const maxRatio = tList.length > 0 ? Math.max(...tList.map((t) => t.ratio || 0)) : 0;
  const maxSeedTime = tList.length > 0 ? Math.max(...tList.map((t) => t.seedingTime || 0)) : 0;
  ...
  const badges: MilestoneBadge[] = [
    ...
    },
    {
      ...
      progress: Math.min(100, (totalUploaded / (100 * 1024 * 1024 * 1024)) * 100),
    ...
    },
    {
      ...
      progress: Math.min(100, (totalUploaded / (1024 * 1024 * 1024 * 1024)) * 100),
    ...
    },
    {
      ...
      icon: "💎",
      tier: "diamond",
      description: "Upload a staggering 10 TB of data to the BitTorrent network.",
      progress: Math.min(100, (totalUploaded / (10 * 1024 * 1024 * 1024 * 1024)) * 100),
    ...
    },
    {
      ...
      icon: "🛡️",
      tier: "silver",
      description: "Keep at least 1 rare torrent alive where total seeders is 2 or fewer.",
    ...
    },
    {
      ...
      icon: "⏳",
      tier: "silver",
      description: "Keep a torrent seeding continuously for 30 days (720 hours).",
  ...
  const overallLevel = Math.max(1, Math.floor((unlockedCount / totalCount) * 10));
  ...
  const rankTitle = rankTitles[Math.min(overallLevel - 1, rankTitles.length - 1)];
 ...
 */
export function calculateTrackerBuffers(torrents: Torrent[] | undefined): TrackerBufferSummary[] {
  const trackerMap: Record<string, { torrents: Torrent[]; uploaded: number; downloaded: number }> =
    {};
  ...
  return Object.entries(trackerMap)
    .map(([tracker, data]) => {
      const ratio =
        data.downloaded > 0 ? data.uploaded / data.downloaded : data.uploaded > 0 ? 10.0 : 0;
