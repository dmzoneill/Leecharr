...
public class TorrentControllerTests : IntegrationTestBase
{
    ...
    {
        var response = await GetJsonAsync<List<TorrentResource>>("/api/v1/torrents");
    ...
    public async Task Torrents_AddMagnetAndPauseResume_Succeeds()
    {
        ...
        var postResponse = await Client.PostAsync("/api/v1/torrents", form);
        ...
        // 2. Pause & Resume
        var pauseResp = await PostJsonAsync($"/api/v1/torrents/{created.Id}/pause", new { });
        pauseResp.StatusCode.Should().Be(HttpStatusCode.OK);

        var resumeResp = await PostJsonAsync($"/api/v1/torrents/{created.Id}/resume", new { });
        resumeResp.StatusCode.Should().Be(HttpStatusCode.OK);

        // 3. Delete
        var deleteResp = await DeleteAsync($"/api/v1/torrents/{created.Id}?deleteFiles=false");
