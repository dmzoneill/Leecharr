...
public class UTorrentWebUiController : ControllerBase
{
                ...
                case "setprio":
                    if (!string.IsNullOrWhiteSpace(effHash))
                    {
                        ...
                        if (target != null && int.TryParse(pVal, out var prio) && int.TryParse(fVal, out var fileIdx))
                        {
                            ...
                            {
                                var internalPrio = MapFromUTorrentPriority(prio);
                    ...
                    if (!string.IsNullOrWhiteSpace(effHash))
                    {
                        ...
                        if (t != null)
                        {
                            ...
                            var fileRows = files.Select(f => new object[]
                            {
                                ...
                                f.Size,
                                f.BytesCompleted,
                                MapToUTorrentPriority(f.Priority),
                            }).ToList();
    ...
    private static int MapFromUTorrentPriority(int priority)
    {
    ...
    private static int MapToUTorrentPriority(int priority)
    {
        return priority switch
        {
            0 => 0,
