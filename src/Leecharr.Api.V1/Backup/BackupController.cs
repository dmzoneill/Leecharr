...
public class BackupController : Controller
{
    ...
    public ActionResult<BackupResource> Create()
    {
        try
        {
            var targetDir = Path.Combine(_appFolderInfo.AppDataFolder, "Backups", "manual");
            ...
            {
                var dbPath = Path.Combine(_appFolderInfo.AppDataFolder, "leecharr.db");
                if (global::System.IO.File.Exists(dbPath))
                {
                    zip.CreateEntryFromFile(dbPath, "leecharr.db");
                }

                var configPath = Path.Combine(_appFolderInfo.AppDataFolder, "config.xml");
            ...
            _logger.Info("Created manual backup archive at {0} ({1} bytes)", zipPath, fi.Length);
            return Ok(backup);
        }
        catch (Exception ex)
        {
            _logger.Error(ex, "Failed to create backup archive");
    ...
    public ActionResult Delete(int id)
    {
        ...
        if (match != null && global::System.IO.File.Exists(match.Path))
        {
            try
            {
                ...
                _logger.Info("Deleted backup archive: {0}", match.Path);
            }
            catch (Exception ex)
            {
                _logger.Warn(ex, "Failed to delete backup archive: {0}", match.Path);
            }
        }
        ...
        try
        {
            using (var zip = ZipFile.OpenRead(backup.Path))
            {
                foreach (var entry in zip.Entries)
                {
                    ...
                    {
                        var destPath = Path.Combine(_appFolderInfo.AppDataFolder, fileName);
                        entry.ExtractToFile(destPath, overwrite: true);
                    }
                }
            }

            _logger.Info("Restored backup archive from {0}", backup.Path);
            return Ok(new { success = true, message = "Backup restored successfully. Please restart Leecharr." });
        }
        catch (Exception ex)
        {
            _logger.Error(ex, "Failed to restore backup archive from {0}", backup.Path);
            return StatusCode(500, new { success = false, message = ex.Message });
        }
    }
}
